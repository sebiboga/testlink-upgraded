...
$validKeys = array("databasetype","databasehost",
                   ...
                   // GitHub OAuth (Issue Tracker connect button)
                   "oauth_github_enable","oauth_github_client_id",
...
// -----------------------------------------------------------------------------
// GitHub OAuth local configuration (optional, new installs only)
// Writes cfg/oauth_github_local.inc.php so the Issue Trackers "GitHub" button
// can authenticate against the GitHub API.
// -----------------------------------------------------------------------------
echo "<br />Writing GitHub OAuth configuration:";
$oauth_ok = write_oauth_github_config();
if($oauth_ok === true) {
  echo "<span class='ok'>OK!</span>";
} elseif($oauth_ok === false) {
  echo "<span class='notok'>Skipped (no credentials provided)</span>";
} else {
  echo "<span class='notok'>Failed!</span></b> - " . $oauth_ok;
  $errors += 1;
}
...
// -----------------------------------------------------------
// Write cfg/oauth_github_local.inc.php (GitHub OAuth for the
// Issue Tracker "GitHub" connect button).
//
// Returns:
//   true                     -> file written OK
//   false                    -> nothing to do (feature disabled / no creds)
//   string (error message)   -> failure
// -----------------------------------------------------------
function write_oauth_github_config()
{
  $enabled = isset($_SESSION['oauth_github_enable']) &&
             (string)$_SESSION['oauth_github_enable'] === 'on';
  $cid  = trim((string)($_SESSION['oauth_github_client_id'] ?? ''));
  $sec  = trim((string)($_SESSION['oauth_github_client_secret'] ?? ''));

  if (!$enabled || $cid === '' || $sec === '') {
    return false;
  }

  // avoid leaving literal placeholders in a live config file
  if (strpos($cid, 'CHANGE') !== false || strpos($sec, 'CHANGE') !== false) {
    return 'client id/secret still contain CHANGE_ placeholders';
  }

  $filename = dirname(__FILE__) . '/../cfg/oauth_github_local.inc.php';
  $body  = "<?php\n";
  $body .= "# Automatically Generated by TestLink Installer - " . date(DATE_RFC822) . "\n";
  $body .= "# Local GitHub OAuth credentials used by the Issue Tracker 'GitHub' button.\n";
  $body .= "# Keep this file private; never commit it to source control.\n";
  $body .= "\$tlCfg->OAuthServers['github']['oauth_enabled']        = true;\n";
  $body .= "\$tlCfg->OAuthServers['github']['oauth_name']           = 'github';\n";
  $body .= "\$tlCfg->OAuthServers['github']['oauth_client_id']      = '" . addslashes($cid) . "';\n";
  $body .= "\$tlCfg->OAuthServers['github']['oauth_client_secret']  = '" . addslashes($sec) . "';\n";
  $body .= "# End Of File\n";

  if (@file_put_contents($filename, $body) === false) {
    return "can't write $filename";
  }
  // do not leave the generated secret world-readable
  @chmod($filename, 0600);
