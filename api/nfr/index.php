<?php
/**
 * Non-Functional Requirements (NFR) BFF API
 * URL: /api/nfr/
 * Plain PHP, no framework, no compilation.
 *
 * Backs the modernized screen gui/templates/requirements/nfrRequirements.html
 * (Refs #1462, #1052). Per-type Non-Functional Requirements management: each
 * test project can define NFR requirements grouped by type
 * (performance / security / usability / accessibility / compatibility /
 * reliability / maintainability), each carrying a target value, a threshold
 * value, a source/reference and a status.
 *
 * Routes (session based, JSON I/O):
 *   GET  ?action=projects                     -> project selector list
 *   GET  ?action=types                        -> NFR type + status domains
 *   GET  ?action=list&tproject_id=N[&type=X]  -> project rows (optionally
 *                                                 filtered by type) + counts
 *   GET  ?action=item&id=N                    -> single row for the edit modal
 *   POST ?action=create {tproject_id,nfr_type,title,...}
 *   POST ?action=update {id,...}
 *   POST ?action=delete {id}
 *
 * Rights model (legacy parity with the Requirements area):
 ...
 *   - unauthenticated -> 401; unknown action -> 400; missing project -> 400
...
function nfrField($input, $key, $maxLen, $default = '') {
    $v = $input[$key] ?? $default;
...
switch ($method) {
    case 'GET':
        ...
        if ($action === 'list') {
            ...
            if ($typeFilter !== '') {
                $where .= " AND r.nfr_type = '" . $db->prepare_string($typeFilter) . "'";
            }

            $rows = (array)$db->get_recordset(
                " SELECT r.*, u.login FROM " . nfrTable() . " r " .
                " LEFT JOIN users u ON u.id = r.author_id " .
                $where . " ORDER BY r.nfr_type ASC, r.id ASC");
            $items = array();
            foreach ($rows as $row) {
                $items[] = nfrRow($row);
            }

            // per-type counts (all rows of the project, irrespective of filter)
            $countRows = (array)$db->get_recordset(
                " SELECT nfr_type, COUNT(*) AS c FROM " . nfrTable() .
                " WHERE testproject_id = " . intval($tpid) .
                " GROUP BY nfr_type");
            $counts = array();
            foreach ($countRows as $cr) {
                $counts[$cr['nfr_type']] = (int)$cr['c'];
            }
    ...
    case 'POST':
        ...
        if ($action === 'create') {
            ...
            $nfrType = nfrField($body, 'nfr_type', 32);
            if (!in_array($nfrType, array_column(nfrTypes(), 'code'), true)) {
                $nfrType = 'performance';
            }
            $title = nfrField($body, 'title', 255);
            if ($title === '') {
                http_response_code(400);
                echo json_encode(array('status' => 'error', 'message' => 'Title is required'));
                exit;
            }
            $reqStatus = nfrField($body, 'req_status', 24, 'proposed');
            if (!in_array($reqStatus, nfrStatuses(), true)) {
                $reqStatus = 'proposed';
            }
            $description = nfrField($body, 'description', 5000);
            $targetValue = nfrField($body, 'target_value', 255);
            $thresholdValue = nfrField($body, 'threshold_value', 255);
            $sourceRef = nfrField($body, 'source_ref', 255);

            $sql = " INSERT INTO " . nfrTable() .
                   " (testproject_id, nfr_type, title, description, target_value," .
                   "  threshold_value, source_ref, req_status, author_id) VALUES (" .
                   intval($tpid) . ", '" . $db->prepare_string($nfrType) . "', '" .
            ...
            $event = new stdClass();
            $event->message = 'NFR requirement created: ' . $title . ' (' . $nfrType . ')';
        ...
        if ($action === 'update') {
            ...
            $existing = $db->fetchFirstRow(
                " SELECT * FROM " . nfrTable() . " WHERE id = " . $itemId);
            ...
            $nfrType = nfrField($body, 'nfr_type', 32, $existing['nfr_type']);
            if (!in_array($nfrType, array_column(nfrTypes(), 'code'), true)) {
                $nfrType = $existing['nfr_type'];
            }
            $title = nfrField($body, 'title', 255, $existing['title']);
            if ($title === '') {
                http_response_code(400);
                echo json_encode(array('status' => 'error', 'message' => 'Title is required'));
                exit;
            }
            $reqStatus = nfrField($body, 'req_status', 24, $existing['req_status']);
            if (!in_array($reqStatus, nfrStatuses(), true)) {
                $reqStatus = $existing['req_status'];
            }
            $description = nfrField($body, 'description', 5000, $existing['description'] ?? '');
            $targetValue = nfrField($body, 'target_value', 255, $existing['target_value'] ?? '');
            $thresholdValue = nfrField($body, 'threshold_value', 255, $existing['threshold_value'] ?? '');
            $sourceRef = nfrField($body, 'source_ref', 255, $existing['source_ref'] ?? '');

            $db->exec_query(
                " UPDATE " . nfrTable() . " SET " .
                " nfr_type = '" . $db->prepare_string($nfrType) . "'," .
        ...
        if ($action === 'delete') {
            ...
            $db->exec_query(
                " DELETE FROM " . nfrTable() . " WHERE id = " . $itemId);
