...
18. **The CI factory (GitHub Actions).** Five autonomous workflows run on this
    ...
    - `implement-task-oldest.yml` (every 2h): picks the OLDEST open `task` issue
      (FIFO, sort:created-asc) and implements the missing feature from scratch on
      its own `task/*` branch. Rulebook: `ai/IMPLEMENT-TASK.md`.
    - `implement-task-newest.yml` (every 2h): picks the NEWEST open `task` issue
      (LIFO, sort:created-desc) and implements the missing feature from scratch on
      its own `task/*` branch. Rulebook: `ai/IMPLEMENT-TASK.md`.
