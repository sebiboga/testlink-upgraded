...
**Precondition:** Fresh DB; logged-in admin session via browser (same-origin CSRF passes). An empty test project exists (created via `POST /api/projects/?action=create` → id=1). Parent id = the project root id itself. Re-executed on a freshly re-imported DB (fixtures recreated: project id=1 "Repro Project").

| # | Test | Expected | Actual | Verdict |
|---|---|---|---|---|
| 1 | `POST /api/testcases/?action=create` with `parent_id` = **test project root id**, name + minimal steps (the pre-fix repro) | HTTP 200 `{"status":"ok","id":<n>,"message":"Test case created"}`; a `tcversions` row with a non-empty `tc_external_id` (auto-incremented); NO DB 500 / no E_WARNING+1064 in `events` | HTTP 200 `{"status":"ok","id":5,"message":"Test case created"}`; `tcversions` id=6 `tc_external_id=2,version=1` (also a first root TC id=2, tcversion id=3 ext=1); events show only info-level login + tproject audit, no E_WARNING/ERROR | PASS |
| 2 | Create test case under a **testsuite** parent (control — unchanged path) | Still HTTP 200, `tcversions` row present | Suite A (suite_create id=8), then TC under it → HTTP 200 ok id=9; tcversion id=10 `tc_external_id=3` | PASS |
| 3 | Create a nested **Suite→Suite→TC** chain under the project root | Sub-suite + test case both HTTP 200; TC `tcversions` row present | Sub Suite (suite_create under id=8) → id=12; Nested TC under it → HTTP 200 ok id=13; tcversion id=14 `tc_external_id=4` | PASS |
| 4 | Event Viewer (`events` table) after steps 1–3 | Zero new `E_WARNING` / `ERROR ON exec_query` rows (log_level=2) produced by the create calls | `events` has only 2 rows, both log_level=16 (info): `audit_login_succeeded` + `audit_testproject_created`; NO log_level=2 errors | PASS |
...
| TC-791.10 | Required checkbox CF group (>=1) | Add checkbox-type (5) exec CF `ecf_exec_check` (possible A/B/C, `cfield_testprojects.required=1`), open form; (a) 0 boxes checked save blocked, (b) check `A` save succeeds | (a) `validateExecCustomFields()` returns ok:false msg \"Execution Check CF is required.\"; (b) returns ok:true and save persists `field_id=2 execution_id=N value='A'` to `cfield_execution_values`. | PASS |
...
| # | Test case | Steps | Expected | Actual |
|---|---|---|---|---|
| TC-820.1 | BFF returns projectIssues for tracker-enabled project | `GET /api/mainpage/index.php?tproject_id=30&tplan_id=31&locale=en` (admin session) | HTTP 200, valid JSON, `status=ok`; `projectIssues` is an array of 49 rows (numbers 503…818, all `state=open`); each row has `id,number,url,title,status,color,labels[],label_colors{}`. The BFF array is ordered by GitHub `created` DESC (rows start at #818 before the client-side id-ASC re-sort). | PASS — 49 rows; first array row is #818; the **rendered** table (id-asc) shows #503 first. Row #503: `url=https://github.com/sebiboga/testlink-upgraded/issues/503`, `status=open`, `color=#e6605e`, `labels=["enhancement"]`, `label_colors={"enhancement":"a2eeef"}`. |
...
| TC-820.9 | DataTable populated + pagination | Open widget; check info line; page through | "Showing 1 to 25 of 49 entries"; page 2 = "Showing 26 to 49 of 49"; table has **4 columns** (Bug id / Summary / Labels / Status); default id-asc order. | PASS — page 1 starts #503…, page 2 #725…#818; 4 columns. |
...
| TC-820.17 | Stored-XSS hardening (title/label escaped) | Inject a fake issue with title+label `<img src=x onerror="window.__xss=1">` through `renderIssues(fake)` (mainPage.html `esc()` helper); inspect `#issuesTable tbody` + `window.__xss` | Raw `<img` must NOT enter the DOM; escaped `&lt;img` present; `window.__xss` stays 0; real 49-issue list still renders. | PASS — `window.__xss=0`, `tbodyHasRawImg=false`, `tbodyHasEscapedImg=true`; reload restores 49 rows. |
| TC-820.16 | Legacy tpl keys (i18n gap closed) | Grep `project_open_issues`, `project_open_issues_hint`, `project_issue_labels` in all `locale/*/strings.txt` | `project_open_issues`/`hint` in 18 files; **`project_issue_labels` was only in 3 (cs_CZ/en_GB/en_US)** — this run added it to the remaining 15 → now 18/18. `issue_summary` pre-existing in 12 files (clean fallback elsewhere). All inserted lines `php -l` clean. | PASS — 18/18 carry all three widget keys after the fix. |

- **Result:** **17/17 PASS** (TC-820.1–820.17)
...
## Regression — Issue #861: Events per Day (last 30 days) line chart has no hover tooltip

**Date:** 2026-09-04
**Screen:** Event Viewer — `gui/templates/eventviewer/eventviewer.html` + `api/eventviewer/index.php`
**Fix:** new `gui/templates/dashio/lib/tl-line-tooltip.js` (`TLLineTooltip.attach`, mirror of `tl-pie-tooltip.js`) hit-tests pointer against the point-dot positions replicated from the Chart.js v1 `Line()` geometry; wired into `renderLine()` (commit, see issue #861 closure comment). The bundled Chart.js is the 2013 v1 build with no native tooltip support, so the same geometry hit-testing shim used for the doughnut chart now covers the line chart. Data prep: 8 fixture events inserted across the last 30 days so the chart has visible points.

| Test | Description | Steps | Expected | Result |
|---|---|---|---|---|
| TC-861.1 | PRE-FIX repro — no tooltip on line chart | Hover the plotted dots of the day chart | WITHOUT the fix: no tooltip element appears (only `#tl-pie-tooltip` exists in DOM) | Confirmed (pre-fix behaviour) |
| TC-861.2 | Tooltip on line-chart dot | Hover dot for 2026-09-04 (value 2) at canvas (644,12) | `#tl-line-tooltip` `display:block`, text `2026-09-04: 2` | PASS |
| TC-861.3 | Tooltip on another dot | Hover dot for 2026-08-10 (value 1) at canvas (144,100) | Tooltip text `2026-08-10: 1` | PASS |
| TC-861.4 | No tooltip on empty area | Hover chart area away from any dot e.g. canvas (904,188) | `#tl-line-tooltip` stays `display:none` | PASS |
| TC-861.5 | Pie tooltip still works | Hover 'Events by Level' doughnut outer ring | `#tl-pie-tooltip` `display:block`, text `ERROR: 6 (75%)` | PASS |
| TC-861.6 | Tooltip hides on mouseleave | Move pointer out of the line canvas | `#tl-line-tooltip` `display:none` | PASS |
...
Result: 19/19 PASS. Files: `gui/templates/requirements/assignReqs.html` (screen), `api/requirements/index.php` (assign-reqspecs / assign-testcases / assign-testsuites / assign-tcase-info / assign-reqs / unassign-reqs / assign-suite-tcases / assign-bulk + `arNeedManageRight` guard), `gui/templates/i18n/*.json` (`reqAssign.*` 37 keys + `footers.assignReqs` in all 10 bundles). ASIDE link `$actions->assignReq` (common.php:1888) → `assignReqs.html`. Legacy reference `lib/requirements/reqTcAssign.php` (rights `rightsAnd=['req_tcase_link_management']`; doSingleTestCaseOperation assign/unassign; bulkAssignLatestREQVTCV). Ref: #1083.

Execution notes (run of 2026-09-06): AR-1083.5 executed against AR-TC-1 after seeding an execution row for its latest tcversion (id 17) via SQL (testplan/build/execution seeded; freed afterwards via DB reset); warning banner + executed badge confirmed. AR-1083.10 verified with an extra empty spec AR-SRS-3 (id 25, added to `tmp/fixtures_1083.php`): selecting it in Bulk mode after picking a suite shows the "No requirements found on this specification." note. AR-1083.17 events sweep: only INFO `audit_login_succeeded` / `audit_user_logout` / `audit_reqv_assigned_tcv` / `audit_reqv_assignment_removed_tcv` from the screen flows — no new Error/Warning rows (events 1-10 are pre-existing fixture-setup artifacts). AR-1083.18: `php -l` clean; Chrome console empty. During this pass a code-review finding (unknown router endpoint returned HTTP 200) was fixed: commit `c5b6611a2` adds `http_response_code(404)` to the requirements router fallback.

---

## Regression — SCREEN-COMPARE keywordsAssign (keywordsAssign.html, Refs #1105)

**Precondition:** browser logged in admin/admin; app `http://localhost:8082`. Fixtures: run `php tmp/fixtures_kwa.php` then `php tmp/fixtures_kwa2.php` → project 1 "KWA Demo Project", keywords alpha(1)/beta(2)/gamma(3)/delta(4), Suite A(2)+nested Suite B(3), cases Case A1(4)/Case A2(7)/Case B1(10), alpha preassigned to A1 latest tcversion; user `kwanr`/`nopass` (role 3 no rights, mapped to project 1), user `kwadesign`/`nopass` (role 4 test designer: `keyword_assignment` WITHOUT `testproject_add_remove_keywords_executed_tcversions`/`testproject_edit_executed_testcases`); plan id 1 + build 2 + execution row on Case A2 latest tcversion (id 8). Screen under test `gui/templates/keywords/keywordsAssign.html?tproject_id=1&tplan_id=1` + BFF `api/keywords/assign.php`.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| KWA-1105.1 | screen loads with project context | open `keywordsAssign.html?tproject_id=1&tplan_id=1` (admin) | title "Assign Keywords to Test Cases"; context "Test Project: KWA Demo Project"; Target level selector (Test Case / Test Suite); Suite dropdown lists "...Suite B" + "Suite A"; no console errors | PASS |
| KWA-1105.2 | testcase mode deep list | Suite A selected → case dropdown | cases listed incl. nested Case B1 (deep scope via `get_testcases_deep`); subtitle "Test Case :: Case B1" | PASS |
| KWA-1105.3 | preassigned keyword shown | select Case A1 | Assigned box = [alpha]; Available = [beta,delta,gamma] (no alpha); hint "Changes will be applied when you press Save (1 item(s))" | PASS |
| KWA-1105.4 | testcase replace save | move beta→Assigned, click Save | toast "Keywords saved"; DB `testcase_keywords` tcversion 5 = (1,2); boxes reloaded with Assigned=[alpha,beta] | PASS |
| KWA-1105.5 | executed badge detection | select Case A2 (has execution row) | `executed` badge visible (admin); Save enabled (admin has canEditExecuted) | PASS |
| KWA-1105.6 | executed block for restricted role | login kwadesign/nopass, Suite A → Case A2 | `executed` badge shown; Save DISABLED; note "Cannot assign to executed test cases in this test plan"; DB write blocked (no rows on tcversion 8) | PASS |
| KWA-1105.7 | suite add deep | suite mode, Suite A, scope=entire subtree, beta→Selected, click Add | toast "…updated" with count; DB: beta on tcversions 5,8,11 (all 3 cases; admin edits executed too) | PASS |
| KWA-1105.8 | suite remove-all | suite mode, Suite A deep, click Remove All | DB `testcase_keywords` empty for tcversions 5,8,11 | PASS |
| KWA-1105.9 | suite add skips executed for restricted role | login kwadesign, Suite A deep, gamma→Selected, Add | toast "updated 2, skipped_executed 1"; DB: gamma on tcversions 5 & 11 only, tcversion 8 untouched (executed guard) | PASS |
| KWA-1105.10 | no-rights user denial | login kwanr/nopass, open screen | note "You do not have permission to assign keywords"; toolbars/pickers/boxes hidden | PASS |
| KWA-1105.11 | no-rights API 403 | kwanr curl POST `/case`, `/suite` AND GET `/suites` `/tcases` `/keywords` | HTTP 403 JSON `{"status":"error","message":"Insufficient rights"}` on all data + mutation routes (right-gating module-level, Refs #1110); `/context` stays 200 with `canAssign:false` to drive the screen note | PASS |
| KWA-1105.12 | error contract | missing tcase_id → 400; missing tsuite_id → 400; unknown tcase 99999 → 404; bogus mode → 400; unknown route `/bogus` → 404 JSON | correct HTTP codes + `{"status":"error","message":...}`; no HTML/die() | PASS |
| KWA-1105.13 | direct API blocked reason | kwadesign POST `/case` {tcase_id:7} | `{"status":"blocked","reason":"executed",...}` HTTP 200 (blocked contract) | PASS |
| KWA-1105.14 | empty-keywords project state | project 13 "KWA NoKw" (no keywords): GET `/keywords` available:[] | modern UI shows "No keywords are defined for this test project yet" and hides boxes (noKwEmpty/otWrap logic); READ ok | PASS |
| KWA-1105.15 | i18n Romanian | locale switcher → Română | title "Atribuire Cuvinte Cheie la Teste"; "Nivel tinta"; "Cuvinte Cheie Disponibile"/"Cuvinte Cheie Atribuite"; all 10 bundles valid JSON, `kwa.*` (29 keys) identical across all | PASS |
| KWA-1105.16 | no new Event Viewer warnings | run all flows then check `events` | only INFO `audit_keyword_assigned_tc`/`audit_keyword_removed` + login/logout rows from screen flows; no new Error/Warning attributable to the screen | PASS |
| KWA-1105.17 | PHP lint + console clean | `php -l api/keywords/assign.php`; Chrome console throughout | "No syntax errors detected"; no Error/Warning console messages | PASS |
| KWA-1105.18 | ASIDE + keywordsView link switched | ASIDE Keywords Assign item; keywordsView "Assign Keywords to Test Cases" button (`goAssign()`) | both open `gui/templates/keywords/keywordsAssign.html?tproject_id=&tplan_id=` — no `frmWorkArea.php?feature=keywordsAssign` in modern producers | PASS |
| KWA-1105.19 | empty suite blocked (Refs #1109) | empty suite 666 "Empty Suite" (no cases) at Test Suite level | note `kwa.emptySuite` ("There are no Test Cases in this Test Suite => keyword assignment is not possible"); Add/Remove/Remove All buttons DISABLED; POST /suite → `{status:"empty",reason:"no_test_cases"}`; switching to non-empty suite re-enables buttons + hides note | PASS |

Result: 19/19 PASS. Files: `gui/templates/keywords/keywordsAssign.html` (screen), `api/keywords/assign.php` (BFF: context/suites/tcases/keywords/case/suite), `gui/templates/i18n/*.json` (`kwa.*` 29 keys identical across 10 bundles), ASIDE `$actions->keywordsAssign` (common.php:2056) → `keywordsAssign.html`, modern `keywordsView.html:421` `goAssign()`. Legacy reference `lib/keywords/keywordsAssign.php` (keyword_assignment right; executed guard via the two testproject rights; useFilteredSet from session edit_mode). Gaps filed: #1106 (useFilteredSet assign-to-filtered-set dropped), #1107 (id/edit deep-link params dropped), #1108 (cleanup legacy). Ref: #1105.

Execution notes (run of 2026-09-06): code-review follow-up fixed a stale-response race in `checkSuiteEmpty()` (captures `sid`+`scope`, drops out-of-order responses; verified with fast 666↔3 toggles — non-empty suite always wins). KWA-1105.6/9 verified with kwadesign (role 4) via curl + browser — the API-level blocked contract (`status:blocked, reason:executed`) confirmed, and the browser shows the disabled Save + executed note. KWA-1105.14 verified with a freshly created project 13 "KWA NoKw" (no keywords) — GET /keywords returns `available:[]`; the screen's `noKwEmpty` card + hidden `otWrap` cover this (logic read; the card was also exercised during the #821 run). The 2 ERROR + 3 WARNING rows present in the events table are fixture-development artifacts (broken SQL in `tmp/fixtures_kwa2.php` iterations — 7 ERROR + 3 WARNING rows, fired_at 20:25-20:26, all BEFORE any screen flow which started 20:38), not screen-originated.

Code-review follow-up (same run): subagent review of `api/keywords/assign.php` + `keywordsAssign.html` — XSS CLEAN (all rendering via `.text()`), SQLi CLEAN (intval/delegated), executed-guard CLEAN, removeall CLEAN. Fixed before close-out (commit e61e48cea / 935f54611): (1) applied the shared `bffSameOriginGuard()` (api/_guard.php) to `assign.php` — same-origin jQuery POST still 200, cross-site Referer POST → 403; the screen relies on `/context` NOT being 403, so the guard is mounted after session start and before routing; (2) null-safe `current(...) ?: []` + `?? 0` on the three executed-flag reads (PHP8); (3) module-level `keyword_assignment` right gate for all non-`/context` routes (closes #1110 data exposure: kwanr GET /suites /tcases /keywords now 403). Event rows re-checked after the fix run: only INFO (level 16) rows.

---

## Regression — Issue #1114: tcCreatedPerUserOnTestProject results table vanishes after 0-row result + re-query

**Precondition:** browser logged in admin/admin; app http://localhost:8082. Fixtures: `php tmp/fixtures_1114.php` → test project 1 "PerUser Demo" (prefix PER), users admin(1)/alicia(2)/bob(3), Suite A(2) + Suite B(3), 7 TCs (ids 4,7,10,13,18,21,24) / 8 tcversions with creation_ts Feb–Apr 2024 (TC Four has v1 bob + v2 alicia). Screen under test `gui/templates/results/tcCreatedPerUserOnTestProject.html?tproject_id=1&tplan_id=0` (session testprojectID=1 via admin default project) + BFF `api/results/index.php`. Window ranges: 2024-02-01..2024-04-30 ⇒ 8 rows; 2024-02-01..2024-02-10 ⇒ 0 rows.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 1114.1 | table renders on normal query | set Start 2024-02-01 00:00, End 2024-04-30 22:00 → Show report | `Report results(8)`; DataTable with 8 rows (User/Test Suite/Test Case/Importance/Created/Last modified + edit icon); footer "8 rows, 3 users, 7 test cases" | PASS |
| 1114.2 | 0-row result shows empty state | narrow End to 2024-02-10 → Show report | `Report results(0)` + "No records found." panel; results table hidden; `resTbl` reference nulled (JS `resTbl === null`) | PASS |
| 1114.3 | PRIMARY BUG — re-query after 0-row re-renders table | widen End to 2024-04-30 → Show report (was: heading `(8)` + footer only, table missing from DOM) | `Report results(8)` AND the DataTable with 8 rows is present again (`document.getElementById("resTable") !== null`; `$("#resTable tbody tr").length === 8`) | PASS (pre-fix: FAIL — `resTbl` stayed truthy after destroy, double-destroy removed the `<table>` node) |
| 1114.4 | 0-rows-first path keeps working | reload screen → 0-row window → Show → then 2024 window → Show | both steps correct: `(0)`+noResults, then 8-row table renders | PASS |
| 1114.5 | normal re-query rows→rows | two Show report clicks with same 2024 window | table re-initialises cleanly both times (8 rows, no console errors) | PASS |
| 1114.6 | Reset then query | Show report (8 rows) → Reset → dates back to defaults, UI cleared → set 2024 window → Show | table renders 8 rows again | PASS |
| 1114.7 | CSV unaffected | GET `/api/results/index.php/csv?start_date=2024-02-01&start_hour=0&end_date=2024-04-30&end_hour=23` | HTTP 200 `text/csv; charset=utf-8`, BOM, header `User,"Test Suite","Test Case",Importance,"Created on","Last modified on","Start Time","End Time"` + 8 data rows | PASS |
| 1114.8 | events clean | after the whole flow check `events` table | no new Error/Warning rows attributable to screen; only INFO audit/login rows (7 fixture-seeding E_WARNINGs at fired_at 1788733146 predate the screen run) | PASS |
...
## SCREEN-COMPARE 1138 — Execution History (execHistory.html vs legacy lib/execute/execHistory.php) — Refs #1138

**Precondition:** app http://localhost:8082, session admin/admin (plus a `nouser`/`nouser` role-3 session in a second context for rights checks). Fixture `tmp/fixtures_exechist.php` (idempotent) → project 135 `EH Demo` (prefix EH), suite 136, TCs TC-EH-one=137, TC-EH-two=140 (with v2=146), TC-EH-never=143; plans A=148 (Build EH-1 open, Build EH-2 closed) B=149 (Build EH-3 open) C=150 inactive; platforms A=11 / B=12; executions ex1..ex6 (ids 19..24, statuses p/f/b/n/p/p; ex3 tester NULL on closed Build EH-2; ex5 carries execution CF `EH_exec_result='PASS-with-CF'`; ex1 carries attachment 'Result log'). Screen: `http://localhost:8082/gui/templates/execute/execHistory.html?tcase_id=140&tproject_id=135`; legacy: `lib/execute/execHistory.php?tcase_id=140`.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 1138.1 | BFF history parity (admin) | GET `/api/execute/index.php?action=history&tcase_id=140` | status ok; 2 executions (ids 24 Plan EH-B / 23 Plan EH-A) with testplan_name/build_name/platform_name/status labels/localized/, run_type manual; ex23 customFields=[{Exec Result: PASS-with-CF}]; ex24 duration 1.25; displayPlatformCol true | PASS |
| 1138.2 | BFF deleted-tester + attachment enrichment | GET history for tcase_id=137 (TC-EH-one) | 4 executions p/f/b/n; ex3 tester_id=0/testers empty (deleted user) can_edit_notes=1; ex1 attachments length 1 (Result log, download_url present); ex4 platform_id 0 → platform_name '' | PASS |
| 1138.3 | BFF neverExecuted + plan filter | GET `?action=history&tcase_id=143` AND `&tcase_id=140&only_active_test_plans=1` | tcase143: executions=[], neverExecuted=true, displayPlatformCol=false; tcase140 active-filter call returns same 2 executions (inactive Plan C carries none) | PASS |
| 1138.4 | Data parity vs legacy screen | Open legacy `execHistory.php?tcase_id=140` side by side (admin) | same 2 rows, same order, same plan/build/platform/status/version/run-mode values; CF value PASS-with-CF present on legacy as 'Exec Result:' | PASS |
| 1138.5 | Screen render | Load modern screen tcase_id=140 | title 'EH-2 - Execution History'; header ext id EH-2 + TC-EH-two; toolbar Test Case EH-2 + Refresh; filters panel; Executions(2); columns Date/Time, Duration, Test Plan, Build, Platform, Executed by, Status, Version, Run mode; both rows PASSED badges; 3 action icons per row (details/print/edit) | PASS |
| 1138.6 | Details panel | Click first row details (comment-dots) | expanded row shows DURATION 3.00, EXECUTION NOTES 'v1 with custom field', CUSTOM FIELDS table label Exec Result → value PASS-with-CF | PASS |
| 1138.7 | Client filters + reset | Build filter = Build EH-1 → Apply; then Tester = Testlink Administrator; then Reset | (1) match, Build EH-1 row only; tester filter narrows further; Reset returns Executions(2) | PASS |
| 1138.8 | Active-plans server toggle | Toggle 'Display only active test plans' | network request `?action=history&tcase_id=140&only_active_test_plans=1` (200); existing client filters preserved | PASS |
| 1138.9 | Never-executed warnbox | Load tcase_id=143 | warn box 'This test case has never been executed.'; filter selects empty; footer present | PASS |
| 1138.10 | Action links middleware | Click print icon (exec 24); click edit icon (exec 24) | print opens `execPrint.html?id=24`; edit opens `editExecution.html?exec_id=24&tcversion_id=146&tplan_id=149&tproject_id=135` with notes 'v2 on plan B' + Exec Result CF input | PASS |
| 1138.11 | No-rights user parity | Session `nouser` load tcase_id=140 (modern + legacy) | both show the same 2 rows and NO edit icon (legacy grant gate / modern can_edit_notes=0); print/details still present | PASS |
| 1138.12 | Footer text fix | Read `exechist.footer` in all 10 bundles + rendered footer | 'Use a row's edit button to update execution notes' (no 'open the legacy screens' anywhere); bundle JSON valid | PASS |
| 1138.13 | Console + events clean | Chrome console during all tests; `SELECT * FROM events WHERE log_level IN (1,2)` after | no JS Error/Warning; no NEW Error/Warning events after final fixture run + browser pass (3 log_level=2 rows are fixture-dev artifacts fired before the fixture's final fixes — cfield_mgr.class.php:1067 scalar link_to_testproject, build.class.php:125 early createFromObject) | PASS |

Result: 13/13 PASS. Fix applied: `gui/templates/i18n/{de,en,es,fr,it,ja,pt,ro,ru,zh}.json` — `exechist.footer` rewritten (was 'Editing operations open the legacy screens' → now 'Use a row's edit button to update execution notes'); verified in-browser. Cleanup tracked: #1139. Screenshots: `tmp/screenshots/exechist_{normal,details,filter,never,nouser}.png`.

## SCREEN-COMPARE 1148 — Test Plan Milestones (planMilestones.html vs legacy lib/plan/planMilestonesView.php / planMilestonesEdit.php) — Refs #1148

**Precondition:** app http://localhost:8082, session admin/admin (plus a `msnorg`/`msnorg` role-3 session in a second context for rights checks). Fixture `tmp/fixtures_1148.php` (idempotent) → project 79 `MS Demo Project` (prefix MS), suite alpha=80 / beta=81, TCs (4 tcversions: MS Login High/Medium, MS Logout Low, MS Search High), plan 94 `MS Plan One`, build 3, milestones: MS Alpha=4 (target 2026-10-07, start 2026-09-12, a/b/c 80/60/50), MS Beta=5 (target 2026-11-06, 90/70/40), MS Gamma=6 (target 2026-12-06, start 2026-09-17, 70/50/30), 4 executions (all today, inside Beta's but outside Alpha/Gamma's start/target window), no-rights user `msnorg` role 3. Screen: `http://localhost:8082/gui/templates/plans/planMilestones.html?tproject_id=79&tplan_id=94`.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 1148.1 | list renders milestones + metrics report (admin) | GET `/api/milestones/index.php/list?tplan_id=94` + load screen | 3 milestones with name/target/start + priority pct; "Status of Milestones" report High/Medium/Low achieved vs expected `(n/m) exp%`; MS Beta 100% (execs inside its window), Alpha/Gamma 0% (execs outside their start date, legacy `getPrioritizedResults` window filter); `rights.canViewEvents=true` for admin | PASS |
| 1148.2 | event-history button shown on edit (admin) | click MS Alpha → edit modal | header shows "Show event history" button (new, Refs #1149); clicking submits hidden `#eventhistory` → opens Event Viewer new tab `?object_id=4&object_type=milestones` showing "Filtered by milestones #4" + the milestone's audit events | PASS |
| 1148.3 | event-history button hidden on create | click "+ New Milestone" | create modal has NO "Show event history" button (legacy `milestone.id>0` parity) | PASS |
| 1148.4 | create milestone + audit | "+ New Milestone" → name "MS Delta", target 2027-01-15, start 2026-12-01, % 75/55/35 → Save | toast `Milestone 'MS Delta' was created`; table + report show MS Delta; events row activity=CREATE object_type=milestones object_id=7 | PASS |
| 1148.5 | duplicate-name validation | "+ New Milestone" → name "MS Beta", future target → Save | toast `A milestone with that name already exists in this test plan: MS Beta`; modal stays open; count unchanged (no duplicate row) | PASS |
| 1148.6 | edit/save milestone + audit | click MS Alpha (→ MS Alpha Updated), change name → Save | toast `Milestone 'MS Alpha Updated' was saved`; row name updated; events row SAVE object_id=4 | PASS |
| 1148.7 | delete milestone + confirm modal + audit | click delete icon on MS Delta → confirm | confirm modal "You are going to delete this milestone. Are you sure?"; after Delete toast `Milestone 'MS Delta' was deleted`; row + report entry gone; events row DELETE object_id=7 | PASS |
| 1148.8 | past-target validation | BFF code read + create with target in past | BFF returns 400 `warning_milestone_date`; screen maps to `ms.msg.pastTargetDate` toast; modal stays open | PASS |
| 1148.9 | no-rights user 403 | session `msnorg` load screen (same plan) | "Insufficient rights" toast; no create/edit/delete chrome (BFF `/list` returns 403) | PASS |
| 1148.10 | i18n + none missing | `ms.showEventHistory` + all `ms.*` keys in 10 bundles; render | all 10 bundle JSON valid; `ms.showEventHistory` translated per locale; screen renders localized | PASS |
| 1148.11 | console + events clean | Chrome console during all tests; `SELECT * FROM events WHERE log_level IN (1,2)` after | no JS Error/Warning; events table contains only expected milestone audit rows (SAVE 4, CREATE 7, DELETE 7) — no new Error/Warning rows | PASS |
...
| 1180.8 | Plan↔project cross-bind hardening | POST quick_result `{tproject_id:50,tplan_id:16,platform_id:0,build_id:30,tcversion_id:5,result:"passed"}` (plan 16 belongs to project 1) | rejected `"Build does not belong to this test project"` — no execution can combine a build and a plan of different projects; no insert | PASS |

Result: 8/8 PASS. Fix = `api/tcassignments/index.php`: the `/quick_result` build validation now queries the real project-scoped column `builds.testproject_id` (4 insertions / 3 deletions, commit `0c97bca0e`) plus a `testplans` JOIN cross-binding the plan to the same project (review hardening, commit `e575f67de`), mirroring `testplan::get_builds()` (lib/functions/testplan.class.php:2274) and sibling BFFs (api/execsetresults:186, api/execute:529). Legacy's unvalidated INSERT was not restored — the BFF's input validation intent is preserved. Screenshots: `tmp/issue-1180-before-click.png`, `tmp/issue-1180-fixed-passed.png`.

## SCREEN-COMPARE 45 — Results by Tester per Build — parity re-verification (Refs #1191)

**Precondition:** app http://localhost:8082, sessions admin/admin + right-less `rbtb_norg2` (role 3). Fixture `php tmp/fixtures_rbtb.php` (idempotent) → project 7 `RBTB` (prefix RBT), suites 2/3 `RBTB Suite One/Two`, TCs RBT-1..RBT-6 (tcversions 2/6/9/12/15/18), plan 28 `PlanRBTB`, builds 1 `RBTB Open Build` (is_open=1) / 2 `RBTB Closed Build` (is_open=0), `user_assignments` (type 1 exec-task): open — rbtb_tester(2)→TC1,TC2,TC5 / admin(1)→TC3,TC4; closed — rbtb_tester→TC1,TC2 / admin→TC4,TC6; executions (tester=assigned, platform 0, durations min): open TC1 p 10.5 · TC2 f 5.25 · TC3 p 2.0 · TC4 b 3.0; closed TC1 p 4.0 · TC4 f 6.5. Empty plan 29 `PlanRBTBEmpty` (builds 0) for the warnings. Screen: `http://localhost:8082/gui/templates/results/resultsByTesterPerBuild.html?tproject_id=7&tplan_id=28`.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 1191.1 | open-build matrix parity (admin) | load screen, toggle off | Open-build card `RBTB Open Build · Progress 80% · Total time (hh:mm:ss) 00:20:45`; rows admin (assigned 2, not_run 0/0.0, passed 1/50.0, failed 0/0.0, blocked 1/50.0, 100.0%, 00:05:00) + rbtb_tester (3, nr 1/33.3, p 1/33.3, f 1/33.3, b 0/0.0, 66.7%, 00:15:45) — byte-identical to BFF curl + legacy `getStatusTotalsByBuildUAForRender` math (round trips) | PASS |
| 1191.2 | closed-builds toggle | check `Show closed builds` | Closed card `RBTB Closed Build · 50% · 00:10:30`; admin (2, nr 1/50.0, f 1/50.0, 50.0%, 00:06:30) + rbtb_tester (2, nr 1/50.0, p 1/50.0, 50.0%, 00:04:00); checkbox stays checked after reload (session persistence) | PASS |
| 1191.3 | no-open-builds warning | `UPDATE builds SET is_open=0 WHERE id=1`; fresh incognito admin session; reload | warning box `There are no open builds` (`rbtb.noOpenBuilds`), no table | PASS |
| 1191.4 | empty-plan warnings | load tplan=29 (no builds) via BFF | `warning_key=no_testers_per_build` (builds project-scoped so plan has none — BFF emits the empty-warning, HTML renders the warn card); with zero open+closed also `no_open_builds` path exercised in 1191.3 | PASS |
| 1191.5 | assignment popup | click `admin` (open build) | `tcAssignedToUser.html?user_id=1&build_id=1&tplan_id=28&tproject_id=7` opens: header `Test Cases Assigned to User`, 2 rows TC3 PASSED / TC4 BLOCKED with suite, platform, spec/exec-history/quick-P/F/B/execute icons | PASS |
| 1191.6 | rights gate (right-less) | session `rbtb_norg2` (role 3, no `testplan_metrics`) via BFF curl + screen | BFF HTTP 403 `{"status":"error","message":"No permission"}`; screen shows localized `Insufficient rights` warn card (guest role 5 IS granted the right by default — verified `role_rights` row role 5 → right 3 — so 403 only for genuinely right-less roles) | PASS |
| 1191.7 | column set + order | BFF payload + render | columns not_run/passed/failed/blocked from results config status display order, server-localized labels; header User/Assigned/Not Run/Passed/Failed/Blocked/Progress [%]/Total time (hh:mm:ss) | PASS |
| 1191.8 | footer + locale | render + locale dropdown | `Generated on ... · Elapsed seconds: X` footer; locale switcher present | PASS |
| 1191.9 | console + Event Viewer clean | Chrome console all pages; `events` log_level IN (1,2) after suite | no JS Error/Warning; only LOGIN audit info rows (level 16) + fixture noise cleaned; **zero** new Error/Warning | PASS |
| 1191.10 | i18n present | all 10 bundles | 13 `rbtb.*` keys valid JSON in en de es fr it ja pt ro ru zh | PASS |
| 1191.11 | status-column alignment (bug found+fixed) | DOM check header vs body cells before/after fix | pre-fix: `<th colspan="2">` ×4 → 12 header cols vs 8 body cells (Progress/Total visually shifted); post-fix (dropped colspan): 8 header = 8 body cells, colspans all 1, row `admin|2|0 0.0%|1 50.0%|0 0.0%|1 50.0%|100.0%|00:05:00` aligns 1:1 with headers User/Assigned/Not Run/Passed/Failed/Blocked/Progress/Total | PASS |

Result: 11/11 PASS — **no data gaps**; one rendering bug found and fixed (Refs #1191): status `<th colspan="2">` vs single count+% `<td>` (12 header vs 8 body cols) — dropped the `colspan` in `gui/templates/results/resultsByTesterPerBuild.html:198`, DOM+visual-verified aligned. The modern screen fully mirrors legacy `resultsByTesterPerBuild.php`: matrix math, closed-builds toggle, both legacy warnings, popup links, rights gating (`testplan_metrics`). SCREEN-COMPARE row 45 marked ✓. Screenshots: `docs/screenshots/rbtb1191_open.png`, `docs/screenshots/rbtb1191_closed.png`, `docs/screenshots/rbtb1191_noopen.png`, `docs/screenshots/rbtb1191_popup.png` (all post-fix).

## SCREEN-COMPARE 48 — Test Cases with Custom Fields — parity analysis (Refs #1204)

**Precondition:** app http://localhost:8082, sessions admin/admin + right-less `tcwcf_norg` (role 3, `tcwcf_norg`). Fixture `php tmp/fixtures_tcwcf.php` (idempotent) → project 45 `TCWCF` (prefix TCW), suites 46/47 `TCWCF Suite One/Two`, TCs TC1..TC6 (tcversions 49/52/55/58/61/64), plan 66 `PlanTCWCF`, empty plan 67 `TCWCF-Empty`, open build 5 `TCWCF Build A (open)` / closed build 6 `TCWCF Build B (closed)`, platforms 3 `TCWCF Linux` / 4 `TCWCF Windows` (linked to plan), execution-time custom field `TCWCF_exec_note` (id 1, `enable_on_execution=1`, linked at TESTCASE node type), tester `tcwcf_tester`=2 (role 5), no-rights `tcwcf_norg`=3 (role 3). Executions with a spread: TC1 build5/plat3 'p' notes+CF(`CF value Alpha`) · TC2 build5/plat3 'f' notes-only (empty) · TC3 build5/plat4 'b' CF-only(`CF value Gamma`) · TC4 build5/plat4 'n' neither → dropped · TC5 build6/plat3 'p' notes+CF(`CF value Epsilon`) · TC6 build6/plat4 'f' neither → dropped. Screen: `http://localhost:8082/gui/templates/results/tcasesWithCF.html?tproject_id=45&tplan_id=66`; BFF `GET /api/reports/index.php?action=tcases_with_cf&tproject_id=45&tplan_id=66`; legacy `http://localhost:8082/lib/results/testCasesWithCF.php?tplan_id=66`.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 1204.1 | row-set parity (hasValue filter) | load modern + legacy side by side | BOTH render exactly 3 rows: TC1/TC3/TC5 (TC2 empty-notes, TC4/TC6 neither → dropped). Modern `d.rows.length===3`, matchCount 3 | PASS |
| 1204.2 | CF column + values | BFF payload + render | `cf_columns=[{id:1,label:'TCWCF Exec Field',name:'TCWCF_exec_note'}]`; header `TCWCF Exec Field`; cell values `CF value Alpha` (TC1), `CF value Gamma` (TC3), `CF value Epsilon` (TC5) | PASS |
| 1204.3 | external id + suite path + icons | render | `TCW-1 : TC1` under `TCWCF Suite One`; `TCW-3 : TC3` under Suite One; `TCW-5 : TC5` under Suite Two; each row has history/execute/edit icons | PASS |
| 1204.4 | platform column (plan has platforms) | render (tplan 66) | `show_platforms=true`; Platform column shown: TC1 TCWCF Linux, TC3 TCWCF Windows, TC5 TCWCF Linux | PASS |
| 1204.5 | status badges + notes | render | TC1 Passed + `notes for TC1`; TC3 Blocked + blank notes (CF only); TC5 Passed + `notes for TC5` (closed-build row included) | PASS |
| 1204.6 | data parity (BFF vs legacy SQL) | compare row 0 of both renders | happy-path: tcase 48/TCW-1/version 1, build 'TCWCF Build A (open)', build_id 5, platform 'TCWCF Linux'/3, tester 'tcwcf_tester', status 'p'/Passed, notes 'notes for TC1', cf 'CF value Alpha' — byte-identical across gens | PASS |
| 1204.7 | empty-plan state | load `tplan_id=67` (TCWCF-Empty) | modern: info box `There are no test cases with a custom field set on execution` (`no_linked_tc_cf`) + `Elapsed` footer, no table; (legacy shows only info-note+timestamp — modern is a helpful superset) | PASS |
| 1204.8 | rights gate (right-less) | session `tcwcf_norg` via BFF + screen | BFF HTTP 403 `{"status":"error","message":"No permission"}`; screen shows generic "Error loading data" toast (BFF rights contract intact; legacy `checkRights` also refuses) | PASS |
| 1204.9 | footer + locale switcher | render | footer `TestLink 2.0.1 - Test Cases with Custom Fields` + `Elapsed: Xs`; locale switcher present | PASS |
| 1204.10 | console + Event Viewer clean | Chrome console; `events` log_level IN (1,2) after browser suite | no JS Error/Warning; **zero** new Error/Warning attributable to screen testing (the stdClass `enable_on_*`/`color` E_WARNINGs at 04:36 are fixture-seed artifacts cleared before browser testing) | PASS |
| 1204.11 | i18n present | all 10 bundles | 14 `tcwcf.*` keys valid JSON in en de es fr it ja pt ro ru zh | PASS |

Result: 11/11 PASS — **no data gaps** (row set + CF values + status + platform + rights byte-identical via shared `get_linked_cfields_at_execution()` + hasValue filter). 4 feature gaps filed as `task` issues (all OPEN): **#1205** ExtTable group-by-Build + toolbar + Date-DESC sort dropped (flat DataTable; legacy `setGroupByColumnName('build')`/`showToolbar`) · **#1206** per-column Platform list filter dropped (legacy `getColumnsDefinition()` `filter='list',filterOptions=$platforms`) · **#1207** Execute icon drops build/platform context (legacy `openExecutionWindow(tc,ver,build,tplan,platform)` → `setting_build`/`setting_platform`; modern `openExecute(tcId,tcverId)` forwards only tcase/tcversion/tplan) · **#1208** info note `info_testCasesWithCF` + "Generated by TestLink on" footer dropped. Cleanup: **#1209** Delete legacy `lib/results/testCasesWithCF.php` + `gui/templates/dashio/results/testCasesWithCF.tpl` (no reportsexport proxy entry for tcases_with_cf; cfg/reports.cfg.php:242 url + asideMenu.php:154 already target modern). SCREEN-COMPARE row 48 marked ✓. Screenshots: `docs/screenshots/tcwcf1204_modern.png`, `docs/screenshots/tcwcf1204_legacy.png` (grouped + toolbar), `docs/screenshots/tcwcf1204_empty.png`.

## Task — Issue #996: Implement export-all-testsuites in projectInfoView (gap vs legacy)

**Precondition:** app http://localhost:8082 (PHP built-in server), DB testlink, login admin/admin. Fixture (idempotent, created via SQL): project 1000 `ExportFixture` (prefix EXP, `is_public=1`, options PHP-serialized) with two direct test-suite children 1001 `SuiteOne` / 1002 `SuiteTwo`; project 2000 `EmptyProject` with NO test-suite children. Admin global role 8 has `mgt_modify_tc` (right 7) and `mgt_modify_product` (12). Screen: `http://localhost:8082/gui/templates/projects/projectInfoView.html?id=<id>`; BFF `GET /api/projectinfo/index.php?action=info&id=<id>`; legacy reference `gui/templates/dashio/testcases/containerView.tpl:150` + `lib/functions/testproject.class.php:778`.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 996.1 | canDoExport flag in BFF | `curl /api/projectinfo/index.php?action=info&id=1000` | payload has top-level `canDoExport:true` (project has >=1 direct testsuite child after excluding testplan/requirement/me — mirrors `testproject.class.php:778`) and `grants.mgt_modify_tc:true` | PASS |
| 996.2 | non-empty project shows Export-all button | open projectInfoView?id=1000 | toolbar shows 'Export all test suites' link (fa-file-export) | PASS |
| 996.3 | button href → project-deep export dialog | click the Export-all link on id=1000 | opens `tcExport.html?tproject_id=1000&containerID=1000&useRecursion=1` → dialog target `ExportFixture`, filename `ExportFixture.testproject-deep.xml` (mode=project) | PASS |
| 996.4 | empty project hides Export-all button | open projectInfoView?id=2000 (no suites) | toolbar has NO Export link (`canDoExport:false`); Refresh/Manage/Dashboard still present | PASS |
| 996.5 | i18n present | all 10 bundles | `piv.exportAll` key valid JSON in en de es fr it ja pt ro ru zh (exact 1-line insert per bundle, no re-sort) | PASS |
...
## Task — Issue #1221/#1223: Fix Export as spreadsheet in resultsTCFlat on plans with >6 active builds

**Precondition:** app http://localhost:8082 (PHP built-in server), DB testlink, login admin/admin. Fixture (idempotent) `php tmp/fixtures_1223.php` → project 1 `RTCF8` (prefix R8, priority enabled) with suite `Suite 1`, test cases 3 `TC-ok` (pass) + 6 `TC-ko` (fail), plan 9 `Plan8`, **8 active builds** B1..B8 (limit default 6), executions for both TCs on every build (16 rows). Screen: `http://localhost:8082/gui/templates/results/resultsTCFlat.html?tproject_id=1&tplan_id=9`; BFF `GET /api/reports/index.php?action=results_flat&tproject_id=1&tplan_id=9`; export gateway `api/reportsexport/index.php?action=results_tc_flat&tplan_id=9&tproject_id=1`. Bug: gateway did not forward `do_action=result`, so legacy resultsTCFlat.php re-entered its launcher branch on >6-build plans and returned HTML instead of XLS. Fix commit 0746160f4.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 1221.1 | launcher shown for >6 active builds | open screen with 8 active builds | warning `Too many active builds. Please select which builds to include. (8 > 6)` + multi-select with B1..B8, Apply Filter button | PASS |
| 1221.2 | apply build subset renders rows | select B1+B2, Apply Filter | DataTable shows 4 rows (TC-ok×B1,B2 + TC-ko×B1,B2), status Passed/Failed, correct build names | PASS |
| 1221.3 | export URL carries build subset | result-mode BFF with `do_action=result&build_set[]=1&build_set[]=2` | `export_xls_url` = `api/reportsexport/index.php?action=results_tc_flat&tplan_id=9&tproject_id=1&buildListForExcel=1,2&build_set[]=1&build_set[]=2` | PASS |
| 1221.4 | gateway redirect adds do_action=result | GET export URL | 303 Location `/lib/results/resultsTCFlat.php?format=3&do_action=result&exportSpreadSheet_x=1&tplan_id=9&tproject_id=1&build_set[]=1&build_set[]=2&buildListForExcel=1,2` | PASS |
| 1221.5 | export returns real XLS (not launcher HTML) | follow redirects (curl -L with session) | final Content-Type `application/vnd.ms-excel; name=resultsTCFlat_RTCF8_Plan8.xls`; `Content-Disposition: attachment; filename="resultsTCFlat_RTCF8_Plan8.xls"`; body = OLE `Composite Document File V2`; NOT H<HTML> | PASS |
| 1221.6 | 3-build subset exports valid XLS | export URL with buildListForExcel=3,4,5 | 303→XLS, OLE file, no HTML | PASS |
| 1221.7 | mail variant also carries do_action | GET `action=results_tc_flat_mail&build_set[]=3` | 303 Location `...?format=6&do_action=result&sendSpreadSheetByMail_x=1&tplan_id=9&tproject_id=1&build_set[]=3` | PASS |
| 1221.8 | Event Viewer clean | `events` log_level IN (1,2) after suite | zero new Error/Warning rows attributable to the screen/export flow | PASS |
...
## Task — Issue #994: Implement testsuite operations panel in projectInfoView (gap vs legacy)

**Precondition:** app http://localhost:8082 (PHP built-in server), DB testlink, login admin/admin. Fixture (created via SQL + BFF during the run): test project `Project Alpha` (id=1, prefix PA, is_public=1) with suites `Zebra Suite`(2), `Apple Suite`(3), `Mango Suite`(4), `Banana Suite`(5, via BFF), `Delta Suite`(6, via modal), `Gamma Suite`(7, via BFF); low-rights user `guestuser` (role 5 guest: rights 3,6,8 — has `mgt_view_tc`, NO `mgt_modify_tc`) created in `users` (auth_method='' so internal DB bcrypt works). Screen: `http://localhost:8082/gui/templates/projects/projectInfoView.html?id=1`; BFF `GET/POST /api/projectinfo/index.php?action={info|new_suite|reorder_suites_alpha}&id=1`. Legacy reference: `gui/templates/dashio/testcases/containerView.tpl:116-163` (testsuite operations panel, gated on `modify_tc_rights`), `lib/testcases/containerEdit.php:729` (`addTestSuite` → `testsuite->create`), `:1381` (`reorderTestSuitesDictionary` → get_children excl. testplan/requirement/testcase/requirement_spec → natsort → change_order_bulk), `:173` (`g_ereg_forbidden` = `/[\|]/i`).

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 994.1 | info payload lists direct suites | GET `?action=info&id=1` as admin | `suites` array = direct testsuite children in display order with id/name/node_order/details (Apple 0, Banana 1, Delta 2, Mango 3, Zebra 4, Gamma 5); no nested testcase rows | PASS |
| 994.2 | admin sees Test Suite Operations toolbar | open projectInfoView?id=1 as admin | `New Test Suite` button + `Reorder test suites (A-Z)` button visible (grants.mgt_modify_tc=true); Test Suites card renders all suites with Order/Name/Details | PASS |
| 994.3 | create suite via modal | click `New Test Suite`, fill `Delta Suite`-style name + details, click Create | modal closes, info line `Test Suite created`, new suite appended (node_order = bottom+1) visible in card + DB (nodes_hierarchy+testsuites) | PASS |
| 994.4 | empty name rejected client-side + server-side | click Create with blank name; BFF POST `{name:'   '}` | UI shows `Please give a name to Test Suite.` (no request); BFF `400 code=empty_name` message `Please give a name to Test Suite` | PASS |
| 994.5 | duplicate name rejected | POST `{name:'Zebra Suite'}`; modal with same | BFF `400 code=duplicate` `There's already a Test Suite with name: Zebra Suite`; modal shows the server message; no row added | PASS |
| 994.6 | forbidden chars rejected | POST `{name:'Bad\|Suite'}` | BFF `400 code=bad_chars` (`g_ereg_forbidden` `/[\|]/i`); no event warning row `config option not available: g_ereg_forbidden` in `events` | PASS |
| 994.7 | reorder A-Z | click `Reorder test suites (A-Z)` + confirm | POST reorder_suites_alpha 200; node_order rewritten Apple/Banana/Delta/Gamma/Mango/Zebra (natsort, case-insensitive); table re-renders | PASS |
| 994.8 | guest cannot operate (UI + BFF) | log in guestuser, open projectInfoView?id=1 | no `New Test Suite`/`Reorder` buttons, no `Manage project` link; read-only card renders; POST new_suite & reorder_suites_alpha → `403 No permission: modify test cases required`; no rows changed | PASS |
| 994.9 | i18n keys in all bundles | grep all 10 bundles | `piv.{newSuite,reorderAlpha,testSuites,suitesHint,noSuites,suiteDetails,newSuiteTitle,newSuiteIn,suiteName,createSuite,suiteNameRequired,suiteCreated,errSuiteDuplicate,errSuiteEmptyName,errSuiteBadChars,errSuiteCreate,reorderConfirm,reordered,errReorder,order}` + `common.working` valid JSON (en de es fr it ja pt ro ru zh), insert-only | PASS |
| 994.10 | Event Viewer clean | `events` table after full suite | only INFO `audit_login_succeeded`/`audit_testproject_created`/LOGIN rows (level 16); zero Error/Warning `log_level IN (1,2)`; php_server.log `[200]`/`[400]`/`[403]` only | PASS |

Result: 10/10 PASS — **feature implemented + verified**. The modern projectInfoView now mirrors the legacy testproject-level testsuite operations panel: `New Test Suite` modal (testsuite::create with duplicate + forbidden-char checks, EVENT_TEST_SUITE_CREATE fired) and `Reorder test suites (A-Z)` (get_children excl. testplan/requirement/testcase/requirement_spec → natsort → change_order_bulk), both gated by `mgt_modify_tc` in UI and BFF, plus a read-only Test Suites card for all viewers. BFF routes `new_suite` + `reorder_suites_alpha` added to `api/projectinfo/index.php`; `info` now returns `suites`. Refs #994. Screenshots: `docs/screenshots/issue-994-testsuite-operations.png`, `docs/screenshots/issue-994-new-suite-modal.png`.

## Task — Issue #998: Align attachment upload/delete rights in projectInfoView with legacy mgt_modify_tc (gap vs legacy)

**Precondition:** app http://localhost:8082 (PHP built-in server), DB testlink. Fixture created via SQL during the run: test project `Task998 Project` (id=1, prefix T998) in nodes_hierarchy+testprojects; scoped users `tconly` (project role on id=1 granting right 7 = `mgt_modify_tc` only) and `prodonly` (global role granting right 12 = `mgt_modify_product` only, per-project grant resolves to null under the TestLink propagation model — `mgt_modify_product` is a `$g_propRights_global` right). Login via isolated incognito contexts. Screen: `http://localhost:8082/gui/templates/projects/projectInfoView.html?id=1`; BFF `POST /api/projectinfo/index.php?action={upload|delete}&id=1`. Legacy reference: `gui/templates/dashio/testcases/containerView.tpl:118-119` (`$bDownloadOnly=false` only when `modify_tc_rights=='yes'`), `gui/templates/dashio/include/attachments.inc.tpl:104,121` (`$attach_downloadOnly` gates upload+delete), `lib/testcases/containerEdit.php` fileUpload/deleteFile gated by `testcase_mgmt`.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 998.1 | tc-only user sees upload panel | log in `tconly`, open projectInfoView?id=1 | "Add attachment" panel visible; table renders; `CAN_UPLOAD` variable true | PASS |
| 998.2 | tc-only user can upload | pick file + title, click Upload | `200 ok`, "Attachment uploaded successfully.", row listed with Download + Delete buttons; row present in `attachments` table (fk_id=1, fk_table=nodes_hierarchy) | PASS |
| 998.3 | tc-only user can delete | confirm Delete on the uploaded row | `200 ok`, "Attachment deleted.", row removed from table + DB | PASS |
| 998.4 | prod-only user (no tc) sees no upload UI | log in `prodonly` (mgt_modify_product="yes", mgt_modify_tc=null via grants), reload screen | `#attUpload` display=none, `CAN_UPLOAD=false`, no Delete buttons; "Manage project" link STILL visible (mgt_modify_product governs that) | PASS |
| 998.5 | prod-only user denied upload at BFF | as prodonly POST `action=upload` with file | HTTP 403 `No permission to upload attachments` (pre-fix this same user passed the mgt_modify_product gate) | PASS |
| 998.6 | prod-only user denied delete at BFF | admin uploads attachment (id=2); as prodonly POST `action=delete&file_id=2` | HTTP 403 `No permission to delete attachments`; attachment still present in DB | PASS |
| 998.7 | admin (both rights) still sees upload | log in admin, open projectInfoView?id=1 | upload panel visible (regression) | PASS |
...
## Task — Issue #989: Implement create-from-existing-test-project (copy) in projectsView create modal (gap vs legacy)

**Precondition:** app http://localhost:8082 (PHP built-in server), DB testlink, admin/admin. Fixture: 2 source projects created via `POST /api/projects/` (id=1 `Source Project Alpha` SPA, id=2 `Source Project Beta` SPB, both optReq=1/0), custom field `cf_severity` (id=1) assigned to project 1 via `cfield_testprojects`. Screen: `http://localhost:8082/gui/templates/projectsView.html`. Legacy reference: `gui/templates/dashio/project/projectEdit.tpl:118-130` (`copy_from_tproject_id` select), `lib/project/projectEdit.php:428-432` (`doCreate` → `$tprojectMgr->copy_as(source,new,user,name,{copy_requirements=>optReq})`). BFF: `POST /api/projects/` now accepts `copy_from_tproject_id`.

| ID | Test case | Repro | Expected | Result |
|----|-----------|-------|----------|--------|
| 989.1 | create modal shows source-project dropdown | click `+ Create Test Project` | "Create from existing Test Project?" select present, options: `-- No --` (default) + each existing project; label from `proj.copyFrom` i18n key | PASS |
| 989.2 | dropdown lists ALL existing projects | open create modal after seeding 2→5 projects | every project appears as an option in the select (verified with 5 projects, both before and after copies existed) | PASS |
| 989.3 | copy path via UI creates cloned content | select `Source Project Alpha`, fill name/prefix (`Copied Project from Alpha` CPA), Save | new project created; `cfield_testprojects` row for field 1 + new project present (custom field assignment copied); BFF returns 200 success | PASS |
| 989.4 | copy path via API with source id | `POST /api/projects/` `{copy_from_tproject_id:1, optReq:1, ...}` | 200 `{success:true,id:4}`; project id=4 has the copied custom-field assignment (`cfield_testprojects.field_id=1,testproject_id=4`) | PASS |
| 989.5 | invalid source id rejected | `POST /api/projects/` `{copy_from_tproject_id:9999,...}` | HTTP 400 `{"error":"Source project not found"}`; no project with that name created afterwards (name remained unique) | PASS |
| 989.6 | no-copy path unaffected (regression) | `POST /api/projects/` without `copy_from_tproject_id` | 200 `{success:true,id:7}`; plain empty project, no cfield_testprojects row | PASS |
| 989.7 | edit modal hides the dropdown | click Edit on any project | select modal shows "Edit Test Project"; no "Create from existing..." row/dropdown | PASS |
| 989.8 | create payload sends copy_from_tproject_id=0 for plain create | open create modal, leave `-- No --`, Save | request body contains `copy_from_tproject_id:0`; project created empty | PASS |
| 989.9 | i18n keys in all bundles | grep `proj.copyFrom`/`proj.noProject` in all 10 locale files + `python3 -m json.tool` | all bundles valid JSON; both keys present in en/de/es/fr/it/ja/pt/ro/ru/zh | PASS |
