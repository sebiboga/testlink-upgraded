<?php
// Fixture for issue #424 browser testing: project + suite + 2 TCs + plan with
// NO platforms + build + TCs linked to plan + 1 PASSED / 1 FAILED execution.
// Run from repo root: php tmp/fixtures_424.php
...
$tcaseMgr = new testcase($tprojMgr->db); // replaced below after db ready
$tcaseMgr = new testcase($db);
$tplanMgr = new testplan($db);

// idempotency: drop previous run's project if present
$old = $tprojMgr->get_by_name('RPT424');
...
$item->name = 'RPT424';
$item->prefix = 'R424';
$item->notes = 'fixture for issue 424';
$item->color = '';
$item->active = 1;
$item->is_public = 1;
$opts = new stdClass();
...
$idTP = $tplanMgr->create('Plan424', 'plan WITHOUT platforms', $idP, 1, 1);
echo "tplan=$idTP\n";

// sanity: plan must have zero platforms
...
$buildId = $buildMgr->create($idTP, 'B1', 'build for 424');
echo "build=$buildId\n";
