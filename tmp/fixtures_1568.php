<?php
// Fixture for #1568: Metrics & Reports navigator screen (resultsNavigator).
// Creates tproject `RSNAV` (prefix `RSN`) + test plan + one test case + a build.
// Run from repo root: php tmp/fixtures_1568.php
...
$item->name = 'RSNAV';
$item->prefix = 'RSN';
$item->notes = 'fixture for issue 1568 (metrics & reports navigator)';
$item->color = '';
$item->active = 1;
$item->is_public = 1;
$opts = new stdClass();
...
$args->notes = 'plan for issue 1568';
$args->active = 1;
$args->is_open = 1;
...
$suiteId = $suiteMgr->create($idP, 'RSNAV Suite', 'suite for issue 1568');
...
$op = $tc->create($suiteId,
                  'RSNAV TC', 'summary for issue 1568', '', '',
                  $userId, '');
...
$buildMgr->create($idT, 'RSNAV Build 1', 'build for issue 1568', 1, 1, '', $idP);
echo "build created\n";
