# Notifications (Refs #896, Refs #1521)
...
## Bugfix (2026-09-16, Refs #1521)

Mark read / Mark all read / Refresh relied on `DataTable().destroy()` +
re-create, which threw `Cannot read properties of undefined (reading 'style')`
(jquery.dataTables.min) on the second render — the table and unread badge kept
showing a **stale "unread" state** although the BFF had persisted `read:true`.
Fixed in commit `b8714ac6d`: the DataTables instance is now **reused** via
`clear() + rows.add() + draw()` and row styling moved into `createdRow` so it
re-applies on every draw (this is the same pattern the ASIDE screens use).

## Screenshots (2026-09-16)
...
### Regression (2026-09-16, Refs #1521)

Full end-to-end pass with live fixtures (project `MQA Project`, plan `MQA Plan`,
TC MQA-101, assignment, 3 milestones, fully-executed plan, bug BUG-100): empty
state, all 4 groups, relative time + due-by + assigner, deep links, sort/search,
mark-selected-read toast + badge flip, mark-all-read → "You are all caught up",
the fixed refresh path, Romanian locale, anon 401 / same-origin 403, clean
console, Event Viewer with only INFO rows. Suite **15/15 PASS**
(`Screen — Notifications center` in `tmp/TLU_Test_Cases.md`).
