...
the initial 2.0.1 popup port. Issue [#1401](https://github.com/sebiboga/testlink-upgraded/issues/1401) restores all three.
...
The legacy popup rendered, in its app-header, an "execInfo" strip + the notes
sections + the type/duration rows that the initial 2.0.1 port dropped:
- **Copy direct link to this execution** — legacy `tplan_id/tcversion_id/
  build_id` feature-id resolution (`execSetResults.php buildExecContext +
  getFeatureID`) exposed by init as `feature_id` + `direct_link`. The popup's
  header **Copy link** button toggles an inline read-only input with the full
  URL (`basehref/ltx.php?item=exec&feature_id=…&build_id=…`, exactly the legacy
  format) and a clipboard copy (navigator.clipboard + execCommand fallback).
- **Print** — header button calling `window.print()`.
- **Import XML Results** — header button opening the modern import popup
  `gui/templates/results/resultsImport.html` with `tproject_id`, `tplan_id`,
  `build_id` and `platform_id` of the current context.
- **Execute and Save Results** (remote exec) — gated on `exec_cfg->
  enable_test_automation` (default DISABLED: button hidden and the BFF action
  answers 403). When enabled, `POST ?action=remote_exec` ports
  `do_remote_execution()` (lib/execute/execSetResults.php:1066): runs
  `executeTestCase()` against the automation server configured via the
  design-time custom fields (`getXMLRPCServerParams`) and, on a `now`
  result, writes an AUTO execution row
  (`TESTCASE_EXECUTION_TYPE_AUTO`, `db->db_now()`, `insert_id()`);
  `configProblems`/`connectionFailure` surface the localized server message.
- **Notes panels** — collapsible Test Plan / Build / Platform boxes (the
  execTest.html Refs #1163 pattern) with the plan+build **design-time custom
  fields** scoped by `show_on_execution=1` (`html_table_of_custom_field_values`
  equivalent), fed by new init fields `tplan_notes/build_notes/
  platform_notes/tplan_cfs/build_cfs`. Empty boxes are skipped.
- **Execution meta row** — "Execution type: Manual/Automated | Estimated
  execution duration: N minutes" (decimal preserved), from init
  `execution_type_label` + `estimated_exec_duration`.
BFF additions (all in `api/execsetresults/index.php`): `esrDirectLink()`,
`esrExecutionTypeLabel()` (uses `TESTCASE_EXECUTION_TYPE_AUTO`), and
`esrNotesPayload()`. i18n: 14 new `esr.*` keys in all 10 locale bundles.
Test cases: TLU suite #1398 (see `tmp/TLU_Test_Cases.md`).
...
- **Assignment line** — legacy `exec_show_tc_exec.inc.tpl:484-491`: when the
  ...
  `testcase::get_version_exec_assignment()` +
  `tlUser::getDisplayName()`, comma-joined exactly like legacy
  `setTesterAssignment()` (execSetResults.php:1195-1225).
- **Assign task to me** — legacy `exec_controls.inc.tpl:52-60`: only shown
  ...
  Default state follows `exec_cfg->exec_mode->assignTaskChecked`
  (default `false`). The checkbox is disabled on closed builds / in
  read-only mode (grants.can_execute=0 hides it entirely for RO users).
...
the existing `exe.*` translations). Test cases: TLU suite #1397 (see
`tmp/TLU_Test_Cases.md`).
## Newer-version handling: auto-bump, hasNewestVersion warning + update-link (issue #1396)
The legacy popup silently executed the newest **plan-linked** version of the
requested test case (auto-bump via `getLinkedItems`, execSetResults.php:2085-2090)
and, when the executed version was NOT the newest one available at all,
displayed a warning + offered "Update Linked TCV To The Latest"
(`hasNewestVersion` @ :1725-1728, `updateTPlanLinkToLatestTCV` @ :83-88,
`exec_show_tc_exec.inc.tpl:455-474`). The initial 2.0.1 port dropped all of it.
Refs #1396 ports it end-to-end:
- **Auto-bump (BFF `init`)** — new `esrNewestVersion()` compares the requested
  version against the newest version **linked to the plan** for that test case
  (join `testplan_tcversions` → `nodes_hierarchy`, max `version`); when a newer
  linked version exists, init resolves it transparently, exactly like legacy.
  Payload additions: `has_newest_version` (0/1 — executed version is NOT the
  newest *available* = `getLatestVersionID` overall), `newest_version_id`,
  `newest_version_number`.
- **Warning + update-link button (UI)** — `#newestVerBox` alert (warning icon,
  `.nv-box` teal/amber palette consistent with the screen) under the card title
...
- **`POST ?action=update_link` (BFF)** — 405 on GET, 409 when the newest
  ...
  (which also re-points existing executions + `cfield_execution_values`).
  Responds `{status,new_tcversion_id,new_version_number}`; the popup
  rewrites its URL to `version_id`/`tcversion_id = new`, reloads context, and the
  warning disappears.
