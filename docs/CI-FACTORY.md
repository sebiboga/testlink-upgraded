...
| `close-done-enhancements.yml` | daily + manual (`dry_run`, `min_age_hours` inputs) | Housekeeping: closes stale `enhancement` tracking issues whose screen is already modernized per `docs/MODERNIZATION-STATUS.md`. Fail-safe triage: never touches issues referenced in the IN PROGRESS section, issues younger than 24h, matches without a verified on-disk `.html` screen, or anything ambiguous — those stay open. |
