...
>
> Last updated: 2026-09-09 — (Quality Objectives `requirements/qualityObjectives.html` + `api/requirements` `quality-objectives` routes recorded as DONE (Refs #1307): ISTQB quality-objective & risk traceability matrix — objective CRUD (name/description/failure-likelihood×business-impact → Low/Med/High/Critical risk), requirement + test-case link management with live latest-execution chips, per-objective passed/failed/blocked/not-run badges, plan filter, BFF `quality_objectives`/`quality_objective_links` tables, `reqs_view` gate, audit events QOBJ_CREATE/UPDATE/DELETE. **Gap closed:** the `qobj.*` TLi18n keys (43 × 10 bundles) had been dropped by a later rebase — restored+validated. Browser-verified: load, create/link/edit/delete objective, plan filter, locale switch, 401 anon. Regression suite 1307 in `tmp/TLU_Test_Cases.md`.)

> Previous: 2026-09-09 — (SCREEN-COMPARE parity for `testcases/tcExport.html` vs legacy `lib/testcases/tcExport.php` (Refs #1323): 4 export modes verified byte-identical XML (single-TC 835 B, project-deep 1061 B) + same default filenames/option states; Markdown superset #853; empty-suite warn box superset; 2 gaps FIXED in-run — file-format documentation link restored (`tcx.fileFormatsDoc`, mirrors reqImport/resultsImport) and `tcx.type.xml`/`tcx.type.md` added to the 8 bundles lacking them (now 32 tcx.* keys in all 10 bundles, validated with python3 -m json.tool); 1 gap OPEN — suite-level export entry point #1325; cleanup task #1324 (delete legacy tcExport). Browser-verified admin on fresh fixtures + Event Viewer clean.)

> Previous: 2026-09-09 — (Severity Configuration `projects/severityConfig.html` + `api/severityconfig` recorded as DONE (Refs #1294): per-project Test Strategy severity scale (Low/Medium/High/Critical) stored in the testproject `options` blob (`severityLevels`), custom labels/descriptions with live severity×urgency preview grid, Reset-to-defaults drops the property, writes gated on `mgt_modify_product` (403), reads session-only (401 anon), project list via `?action=projects`. Browser-verified: load, custom-label save + DB persistence, preview mapping, reset, Event Viewer clean. i18n `sevcfg.*` (36 keys) + `footers.severityConfig` in all 10 bundles. Regression suite 1294 11/11 PASS. ASIDE link `gui/templates/dashio/aside.tpl:107` + guide link → `documentation/bugSeverity.html`.)
...
| 24b | Requirements — Quality Objectives | `requirements/qualityObjectives.html` | api/requirements | #1307, #1280 |
...
- Test Case Bulk Update (`lib/testcases/tcBulkOp.php`) — `gui/templates/testcases/tcBulkOp.html` + `api/tcbulkop/index.php` `GET ?action=init` + `POST ?action=apply` (#1074, DONE). The last standalone legacy screen without a modern replacement: applies a chosen Status/Importance/Execution type to ALL versions of one test case via the legacy `testcase::setIntAttrForAllVersions()` (attr value `>0` only; `-1` sentinel = no change), honoring the `forceFrozenTestcasesVersions` flag (`is_open=0` children skipped unless forced). BFF resolves the owning project by walking the test-case node up `nodes_hierarchy`; **`mgt_modify_tc` gates `apply`** (403 verified with a `<no rights>` user; `init` open to authenticated sessions); `hasRight()` legacy `'yes'` return normalized to a real boolean. Version context (active/frozen counts + open/frozen chips), server-localized domain selects + client-localized "(no change)" sentinel, confirm dialog, success/error toasts, 401/403 fatal warnboxes. **Link switch:** modern `tcView.html` toolbar gained the **Bulk Update** button (`openTcBulk()`, gated on `mgt_modify_tc`) → `tcBulkOp.html?tcase_id=&tproject_id=` popup. i18n `tcbk.*` (28 keys) + `tcview.bulkUpdate` in all 10 bundles. Regression suite TC-1074 15/15 PASS; Event Viewer clean; wiki + docs mirror + screenshots.
