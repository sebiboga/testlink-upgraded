...
* **Since #624** a row counts as *Latest* only under the exact legacy rule of
  `testplan::get_linked_and_newest_tcversions()`: among ACTIVE siblings take
  those with node id > linked id, newest = max-id candidate, and require its
  VERSION number > linked version number (node-id order ≠ version order after
  imports). The *Linked version inactive* badge outranks the green Latest badge.
...
|---|---|
| GET `/api/plans/index.php/update-linked?tproject_id=&tplan_id=` | Context payload: project/plan info, suites with updatable counts, per-item linked/newest/target versions + executed qty + state (`ok` / `empty` / `pick`). **Since #624:** `tplan_id=0` with a valid `tproject_id` no longer 400s — it returns the picker payload (`tplans`, `tplan:null`, empty `suites`/`items`) so the screen boots into plan-selection mode |
...
(`{n}` placeholder). Since #624 `apiFail()` maps server `error_code`s to
localized toasts: `NO_TPLAN→uptc.errNoTplan`,
`NO_TPROJECT/BAD_TPLAN→uptc.errNoTproject`, `NO_SELECTION→uptc.noSelection`;
unknown codes fall back to the raw server message.

## 6a. Issue #624 Fix — is_latest semantics & boot dead-end

**Root cause chain**
1. `updTcItems()` derived `is_latest` from node-id ordering only
   (`max(active sibling id) <= linked id`) — legacy requires the additional
   version-number condition (`testplan.class.php:1150,1158`). A link on
   version 5 whose v3 sibling had a higher node id was offered a "downgrade as
   update"; bulk preview disagreed with the bulk action.
2. Badge priority rendered green *Latest* before checking `linked_active`,
   masking inactive-linked rows.
3. GET with `tplan_id=0` hard-failed 400 `NO_TPLAN`, leaving an empty picker
   (dead end) plus a raw English toast (`apiFail()` had no error_code map).
4. Minor: footer printed bare `"0 "`; `getDBTables()` called inside the
   per-item loop of POST `/latest`.

**Fix approach** — replicate the legacy candidate rule in `updTcItems()`
(active sibling with id>linked AND version>linked ⇒ updatable; newest = max-id
such candidate), keep all active non-linked siblings as manual targets
(`tideUpForGUI()` parity); add context-only GET mode returning `state:'pick'`
with the plan list; reorder badge checks (inactive first); map error_codes to
TLi18n keys added to all 10 bundles; localize the 0-row footer; hoist
`getDBTables()`.
...
Regression Suite 624 (11 cases R1–R11, all PASS) covers the #624 fixes:
id-order vs version-order edge (v5-linked row stays Latest), inactive-badge
priority, preview==bulk-action agreement, pick-mode boot, localized error
toasts and footer.
