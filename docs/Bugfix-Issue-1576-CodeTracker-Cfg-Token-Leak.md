...
- `api/codetracker/index.php` — `$canManage` + cfg gating in `trackerToJSON`
  (+21/−7 lines vs the base `3ebd8ebb9`).
- `tmp/TLU_Test_Cases.md` — `Regression — Issue #1576` suite (6/6 PASS).
- `CHANGELOG` — `[KEY BUGFIX - SECURITY] - #1576` entry.
...
rebased onto `3ebd8ebb9`; commits `ab77164c9` fix + `dd78d6319` suite +
`4d043e574` docs); regression suite 6/6 PASS; Event Viewer clean (the 16
pre-existing `tlCodeTracker.class.php` WARNING rows were generated by the
repro fixture with the unregistered type-6 tracker and already filed). A
related out-of-scope finding (BFF accepts unregistered tracker `type` →
E_WARNING noise) was filed separately as issue #1577.