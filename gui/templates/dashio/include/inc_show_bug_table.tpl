	...
	<tr>
        ...
        {if $gui->issueTrackerIntegrationOn}
          <td>
      		...
      		<span 
 title="{$labels.bug_link_tl_to_bts}" style="border:none">{$tlImages.bug_link_tl_to_bts}</span></a>
          </td>
        {/if}
		...
		{if $can_delete}
		  <td class="clickable_icon">
		  	<span class="clickable" onclick="delete_confirmation('{$exec_id}-{$bug_elem.tcstep_id}-{$bug_id|escape:'javascript'|escape}','{$bug_id|escape:'javascript'|escape}',
 '{$l10nb.delete_bug}','{$l10nb.del_bug_warning_msg} ({$l10nb.bug_id} {$bug_id})',deleteBug);" style="border:none" title="{$l10nb.delete_bug}" 
>{$tlImages.delete}</span></td>
		{/if}
	</tr>
	{/foreach}
</table>
