...
<div class="container-fluid" id="{$whoaim}">
 ...
 onclick="document.getElementById('relations_{$tcversion_id}').scrollIntoView();">{$tlImages.relations}</span>       
      {/if} 
 ...
 onclick="showHideByDataEntity('ghostTC');">{$tlImages.ghost_item}</span>

      <span class="clickable" 
 title="{$inc_tcbody_labels.display_author_updater}"
 onclick="showHideByDataEntity('createUpdate');">{$tlImages.activity}</span>
  </div>
