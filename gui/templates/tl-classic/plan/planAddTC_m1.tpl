         ...
         {if ($gui->full_control && $ts.testcase_qty gt 0) || $ts.linked_testcase_qty gt 0}
            <table cellspacing="0" border="0" style="font-size:small;" width="100%">
              <tr style="background-color:blue;font-weight:bold;color:white">
                  ...
                  {if $gui->full_control}
    			          <span class="clickable" onclick='cs_all_checkbox_in_div("{$div_id}","{$add_cb}","add_value_{$ts_id}");' title="{$labels.check_uncheck_all_checkboxes}">{$tlImages.toggle_all}</span>
        			    {else}
        			     &nbsp;
    		    	    {/if}
    			     </td>
               ...
               {if $ts.linked_testcase_qty gt 0}
    				      <td>&nbsp;</td>
    				      <td>
    				      <span class="clickable" onclick='cs_all_checkbox_in_div("{$div_id}","{$rm_cb}","rm_value_{$ts_id}");' title="{$labels.check_uncheck_all_for_remove}">{$tlImages.disconnect}</span>
    				      </td>
    				      <td align="center">
                ...
                {if $is_active || $linked_version_id != 0}  
                    ...
                    {/if}
                      ...
                      {/if}
        			        ...
        			        <td>
							<span class="clickable" onclick="javascript:openExecHistoryWindow({$tcase.id},undefined,{$gui->tproject_id});" title="{$labels.execution_history}">{$tlImages.history_small}</span>
