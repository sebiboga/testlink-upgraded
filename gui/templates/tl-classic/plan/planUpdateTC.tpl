    ...
    <input type="hidden" name="update_all_value"  id="update_all_value"  value="0" />
  	...
  	{foreach from=$gui->items item=ts}
  	  ...
  	  <div id="{$div_id}"  style="margin:0px 0px 0px {$ts.level}0px;">
  	    <h3 class="testlink">
        {if $item_number ==1}
  	    <span border="0" title="{$labels.check_uncheck_all}" onclick="cs_all_checkbox_in_div('updateTcForm','{$update_cb}','update_all_value');">{$tlImages.toggle_all}</span>
        {/if}
          ...
          <table border="0" cellspacing="0" cellpadding="2" style="font-size:small;" width="100%">
            <tr style="background-color:blue;font-weight:bold;color:white">
  			     <th class="clickable_icon">
  			         <span title="{$labels.check_uncheck_all_checkboxes}" onclick='cs_all_checkbox_in_div("{$div_id}","{$update_cb}","update_value_{$ts_id}");'>{$tlImages.toggle_all}</span>
  			     </th>
              ...
              {* ------------------------------------------------ *}
     		  {if $is_linked }
					...
					<td>
						...
						<span class="clickable" onclick="javascript:openTCaseWindow({$tcase.id});" title="{$labels.design}">{$tlImages.edit}</span>
