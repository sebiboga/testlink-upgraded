...
<form method="post" action="lib/usermanagement/usersEdit.php" class="x-form" name="useredit" 
  ...
  <span style="margin-left:5px;" class="clickable" onclick="showEventHistoryFor('{$user_id}','users')" title="{$labels.show_event_history}">{$tlImages.help}</span>
  {/if}
  </legend>
  <table class="common">
    <tr>
   ...
   {/if}
    ...
    <tr>
      ...
      <td>
        ...
        <span title="{$labels.show_calender}" onclick="showCal('expiration_date-cal','expiration_date','{$gsmarty_datepicker_format}');">{$tlImages.calendar}</span>


        <span title="{$labels.clear_date}" onclick="javascript:var x = document.getElementById('expiration_date'); x.value = '';">{$tlImages.clear}</span>
