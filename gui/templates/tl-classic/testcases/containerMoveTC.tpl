    ...
    <input type="hidden" name="add_value_memory"  id="add_value_memory"  value="0" />
        ...
        <table class="simple" id="item_view">
          ...
          <th class="clickable_icon">
			         <span onclick='cs_all_checkbox_in_div("move_copy_checkboxes","tcaseSet_","add_value_memory");' title="{$labels.check_uncheck_all_checkboxes}">{$tlImages.toggle_all}</span>
			    </th>
        ...
        {foreach from=$gui->testcases key=rowid item=tcinfo}
            <tr>
                ...
                <td>
                    <span class="clickable" onclick="javascript:openExecHistoryWindow({$tcinfo.tcid});" title="{$labels.execution_history}">{$tlImages.history_small}</span>
                    <span class="clickable" onclick="javascript:openTCaseWindow({$tcinfo.tcid});" title="{$labels.design}">{$tlImages.edit_icon}</span>
