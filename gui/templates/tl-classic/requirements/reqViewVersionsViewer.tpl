  ...
  <td>
    ...
    {else}    
        ...
        {else}
          &nbsp;&nbsp;&nbsp; 
        {/if}
        <span class="clickable" onclick="javascript:openExecHistoryWindow({$args_req_coverage[row].id});" title="{$labels.execution_history}">{$tlImages.history_small}</span>
        <span class="clickable" onclick="javascript:openTCaseWindow({$args_req_coverage[row].id});" title="{$labels.design}">{$tlImages.edit_icon}</span>
    ...
    <form style="display: inline;" id="reqAddTestCase_{$req_version_id}" name="reqAddTestCase_{$req_version_id}" 
      ...
      <span class="clickable" onclick="javascript:toogleShowHide('addTestCase');" title="{$labels.addLinkToTestCase}">{$tlImages.add}</span> 
