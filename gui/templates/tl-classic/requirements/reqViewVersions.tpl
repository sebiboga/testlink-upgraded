...
{section name=idx loop=$gui->current_version}
  ...
  {if $gui->display_path}
      ...
      {/foreach}
  {/if}
  <span class="clickable" onclick="javascript:toogleShowHide('control_panel_{$latestReqVersionID}','inline');" title="{$labels.actions}">{$tlImages.cog}</span>
      ...
      {foreach item=relation from=$gui->req_relations.relations}
        ...
        <tr>
          ...
          {if $gui->req_relations.rw}
			...
			{else}
				  <span style="border:none;" title="{$labels.img_title_relation_frozen}">{$tlImages.delete_disabled}</span>
			{/if}
		  {/if}
          </td>
        </tr>
        ...
        {foreach from=$gui->other_versions[idx] item=my_req key=rdx}
            ...
            {include file="inc_show_hide_mgmt.tpl" 
              ...
              <div id="{$div_id}" class="workBack">
                ...
                <span class="clickable" onclick="javascript:toogleShowHide('control_panel_{$reqVersionID}','inline');" title="{$labels.actions}">{$tlImages.cog}</span>
