          ...
          <!-- {include file="attachments_simple.inc.tpl" attach_id=0} -->
              ...
              {if $args_save_type == 'bulk'} {$args_labels.test_exec_result} {else} &nbsp; {/if}
                ...
                {if $tlCfg->exec_cfg->features->exec_duration->enabled}	
                  <br />	
                  <span title="{$args_labels.execution_duration}">{$tlImages.execution_duration}</span>
                ...
                {if $args_save_type == 'single'}
                  ...
                  {if $tc_exec.assigned_user == ''}
                   <span title="{$args_labels.assign_exec_task_to_me}">{$tlImages.assign_task}</span>
                  ...
                  {if $tlCfg->exec_cfg->exec_mode->new_exec == 'latest'}
                    {$addBR=1}
                   <span title="{$args_labels.copy_attachments_from_latest_exec}">{$tlImages.copy_attachments}</span>
                  ...
                  {if $gui->tlCanCreateIssue}
                    {$addBR=1}
                    <span title="{$args_labels.bug_create_into_bts}">{$tlImages.bug_create_into_bts}</span>
                  ...
                  {/if}
                   ...
                   {foreach key=kode item=ikval from=$gui->execStatusIcons}
                     {$in = $ikval.img}
                     <span title="{$ikval.title}" name="fastExec{$kode}[{$tcversion_id}]" id="fastExec{$kode}_{$tcversion_id}" onclick="javascript:saveExecStatus({$tcvID},'{$kode}');">{$tlImages.$in}</span>&nbsp;
                   {/foreach}  
                   <br />
                   <br />
                   ...
                   {foreach key=kode item=ikval from=$gui->execStatusIconsNext}
                     {$in = $ikval.img}
                     <span title="{$ikval.title}" name="fastExecNext{$kode}[{$tcversion_id}]" id="fastExecNext{$kode}_{$tcversion_id}" onclick="javascript:saveExecStatus({$tcvID},'{$kode}','',1);">{$tlImages.$in}</span>&nbsp;
                   {/foreach}  
                   <br />
                   <br />
