...
*}
      ...
      {foreach item=tc_exec from=$gui->map_last_exec name="tcSet"}
        ...
        <input type="hidden" id="version_number_{$tcversion_id}" name="version_number[{$tcversion_id}]" value='{$version_number}' />
          ...
          <td>
                  <span class="clickable" onclick="javascript:openExecHistoryWindow({$tc_exec.testcase_id});" title="{$labels.execution_history}">{$tlImages.history_small}</span>
                  <span class="clickable" onclick="javascript:openExecutionWindow({$tc_exec.testcase_id},{$tcversion_id},{$gui->build_id},{$gui->tplan_id},{$gui->platform_id});" title="{$labels.execution}">{$tlImages.exec_icon}</span>
                  <span class="clickable" onclick="javascript:openTCaseWindow({$tc_exec.testcase_id},{$tc_exec.id});" title="{$labels.design}">{$tlImages.edit}</span>        
